...
\begin{definition}[cylindrification]\label{cyli}
	...
	$\exists: V \times A \rightarrow A$ such that for all $x \in V$
...
\begin{definition}[polyadification]
	...
	\begin{enumerate}
        ...
        = s(\tau, a)$,
        \item $\forall a \in A.\ \exists_x s_\sigma a =
  			\begin{cases}
    			s_\sigma \exists_y a \ if \ \sigma^c(x) = {y} \\
        		s_\sigma a \ if \ \sigma^c(x) = \emptyset.
  			\end{cases}$	
    \end{enumerate}
\end{definition}
...
\begin{lemma}
	\label{lemmaSubs0}
	Let $\sigma \in F(V)$ be invertible and $x \in V$. Then it holds
	\begin{enumerate}
		\item $\forall a\in A.\ s_\sigma \exists_x a = \exists_x a$,
		\item $\forall a\in A.\ \sigma(x) = y \land y \not \in sv(a) \implies \exists_x a =
		\exists_y s_\sigma a$,
		\item $\forall a \in A.\ z \not \in (sv(\sigma) \cup \sigma(sv(\sigma)))
		\implies s_\sigma \exists_z a = \exists_z s_\sigma a$.
	\end{enumerate}
\end{lemma}
...
Let $\phi \in F(V)$. 
Substitution $[^y/_x]: \mathcal{A} \rarrow \mathcal{A}$ is defined by structural induction as 

\begin{itemize}
	\item $s_\phi \ostop = \ostop$
	\item $s_\phi\tell(c) = \tell(s_\phi c)$
	\item $s_\phi p(w) =  p(\phi(w))$
	\item $s_\phi (\ask(c) \rightarrow A) = \ask(s_\phi(c)) \rightarrow s_\phi A$
	\item $s_\phi (\exists_w A)  = 
		\begin{cases} 
			\exists_w A & \text{if $w \not \in sv(a)$} \\
            \exists_z (s_\psi s_\phi A) & \text{with $\psi(w) \not \in sv(s_\psi a)$}
        \end{cases}$
\item $s_\phi(A_1 \parallel A_2)  = (s_\phi A_1 \parallel s_\phi A_2)$
\end{itemize}
\end{definition}
...
\begin{table}  %\hfil5
  ...
  \mbox{\bf A2}& $\frac {\displaystyle sv(\sigma) \cup sv(c) \cup fv(A) \subseteq \Delta \wedge c \leq \sigma}{\displaystyle
    ...
    &\mbox{   }&\mbox{   }&
    \\
  \mbox{\bf A3}& $\frac {\displaystyle sv(\sigma) \cup \{\phi(x)\} \subseteq \Delta \wedge \displaystyle p(x) = A \in  \mathcal{P} \wedge \phi(x) = y }
  {\displaystyle\langle p(y),\sigma\rangle \mapsto_\Delta \langle  s_\phi A, \sigma\rangle}$ 
  &\bf{Rec}&
    \\
   &\mbox{   }&\mbox{   }&
  \\
    ...
    \wedge w = \phi(x) \not \in \Delta }
    {\displaystyle\langle \exists_x A,\sigma\rangle \mapsto_\Delta \langle s_\phi A, \sigma\rangle}$
    &\bf{Hide}&
  \end{tabular}
  \end{center}
