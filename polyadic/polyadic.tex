...
%(or $\otimes$-finite) 
if whenever $a \leq \bigotimes_{i \in I} a_i$ %for some family $I$
...
\vspace{-.5cm}
\def\odiv{\; {\ominus\hspace{-7.1pt} \div} \;}
\def\odivvv{\; {\ominus\hspace{-7.1pt} \div} \;}
\begin{table}
\begin{center}
	\scalebox{0.9}{
		\begin{tabular}{llll}
			$\frac {\displaystyle \langle A, \sigma' \otimes \exists_x \sigma_0
				\rangle \mapsto \langle B, \sigma'' \rangle \ \ \ \mbox{  \textit{with}  } \sigma_0 = \sigma \odiv \exists_x \sigma'} 
			{\displaystyle\langle \exists^{\sigma'}_x A,
				\sigma \rangle \mapsto \langle \exists^{\sigma_1}_x B, \sigma_0 \otimes \exists_x \sigma_1\rangle \ \ \ \mbox{  \textit{with}  } \sigma_1 = \sigma'' \odiv \exists_x \sigma_0}$
			...
			$\frac {\displaystyle \langle A[^z/_x], \sigma'[^z/_x] \otimes \sigma_0 \rangle \xrightarrow{\;  \alpha \;}
				\langle B,  \sigma'' \rangle\ \ \ \mbox{  \textit{with}  } \sigma_0 = \sigma \odiv \exists_x \sigma',\, 
				                     z \not\in fv(A) \cup sv(\sigma)\cup sv(\sigma') \cup sv(\alpha)}
			{\displaystyle\langle \exists^{\sigma'}_x A, \sigma\rangle \xrightarrow{\;  \alpha \;}
				\langle \exists^{\sigma_1}_z B, \sigma_0 \otimes \alpha \otimes \exists_z\sigma_1 \rangle
				\ \ \ \mbox{  \textit{with}  } \sigma_1 = \sigma'' \odiv (\alpha \otimes \sigma_0)}$
			&\bf{L-Hide}&
		\end{tabular}
	}
\end{center}
