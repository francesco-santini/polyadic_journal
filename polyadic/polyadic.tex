...
\begin{definition}[cylindrification]\label{cyli}
	A cylindric operator $\exists$ over $\mathbb{S}$ and $V$ ia family of monotone, 
...
\begin{definition}[polyadification]
	\label{def:poly}
	A polyadic operator $s$ for $\exists$ is a a family of operators $s_\sigma: A \rightarrow A$
...
\begin{lemma}
	\label{lemmaSubs0}
	Let $\sigma \in F(V)$ invertible, $a \in A$, and $x, y \in V$ $x\in sv(\sigma)$. Then it holds
	\begin{enumerate}
		\item if $s_{\sigma} \exists_x a = \exists_x a$
...
\begin{itemize}
	...
	\item $[^y/_x] (\exists_w A)  = 
		\begin{cases} 
			\exists_w A & \text{if $w = x$} \\
            \exists_z ([^z/_y] [^y/_x] A) & \text{for z fresh if $w = y$} \\
            \exists_x ([^y/_x] A) & \text{otherwise}
        \end{cases}$
...
\begin{remark}
Note that terms are $\alpha$-convertible: two terms which differ only for hidden variables can be considered equivalent. Indeed, as $sv([^y/_x]) = x$ and $x \not \in fv(\exists_x A)$, we have $\exists_x A = [^y/_x] \exists_x A$. If $y \not \in fv(A)$, we can swap the two operators, thus obtaining 
\[y \not \in fv(A) \implies \exists_x A = \exists_y [^y/_x] A\]
which precisely induces $\alpha$-conversion.
\end{remark}
...
\begin{table}  %\hfil5
  ...
  &\bf{Rec}&
    ...
    \Delta \wedge w \not \in \Delta }
    {\displaystyle\langle \exists_x A,\sigma\rangle \mapsto_\Delta \langle A[^w/_x],
    \sigma\rangle}$
    &\bf{Hide}&
    \\
   &\mbox{   }&\mbox{   }&
...
%   &\mbox{   }&\mbox{   }&
%  \\
%    \mbox{\bf A5}& $\frac {\displaystyle \langle A, \sigma' \otimes \exists_x \sigma_0
%				\rangle \rightarrow \langle B, \sigma'' \rangle \ \ \ \mbox{  \textit{with}  } \sigma_0 = \sigma \odiv \exists_x \sigma'} 
%			{\displaystyle\langle \exists^{\sigma'}_x A,
%				\sigma \rangle \rightarrow \langle \exists^{\sigma_1}_x B, \sigma_0 \otimes \exists_x \sigma_1\rangle \ \ \ \mbox{  \textit{with}  } \sigma_1 = \sigma'' \odiv \exists_x \sigma_0}$
%    &\bf{Hide-local}&
  \end{tabular}
  \end{center}
...
In axiom {\bf A5} In we consider instead local variables, where the hiding operator carries some information on the variables it abstracts.
...
Thanks again to the residuation operator, the rule for the extended hidings can be defined as {\bf Hide-local}. The intuition is that variable $x$ may be local to a component $\exists_x \sigma'$ of the store $\sigma$, yet visible at a global level: we must then evaluate $A$ in the store
