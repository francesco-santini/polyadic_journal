...
%
The choice of polyadic algebras in place of classical cylindric ones
 simplifies the structure of the set of preference values by removing the need of diagonals, which are constants used for modelling parameter passing or variable substitution. 
%
Diagonals are replaced with a family of polyadic operators, and we investigate how these operators interact with the residuated monoid structure representing the set of preference values. 
%
...
which are used for modelling parameters passing and variable substitution.
...
To evaluate preferences, the set $A$ is equipped with a semi-lattice structure while, to make possible the combination of soft constraints, one usually exploits a monoidal operator over $A$. 
Therefore, $A$ turns out to be an idempotent semiring (also called tropical or diod). 
%
...
A further operation is required over the set $A$, in order to model the removal of constraints, 
...
which are not always met in actual case studies. Moreover, the diagonal operations present some serious issues in finding a compact representation.
...
%
Finally, in Section~\ref{sec:polynomialsoftconstraints} we introduce a set of constraints that can be exploited for polynomials representation 
...
The word ``constraint'' occurs in the titles of 40 publications coauthored by Catuscia. Between 1991 and 1997, most of these papers were devoted to the semantics~\cite{boer1} and analysis~\cite{de1997proving} of Constraint Logic Programming and Concurrent Constraint Programming. In 2001, the latter language was extended to Temporal Concurrent Constraint Programming~\cite{nielsen2002temporal,palamidessi2001temporal}.
...
%
They are mostly drawn from~\cite{jlamp17} but, the material from Proposition~\ref{reabs}
to Example~\ref{nodist2} is, to the best of our knowledge, original.
%
...
for the missing proofs as well as for a detailed introduction and a comparison with other proposals.
...
\begin{definition}[partial order]
	...
	%
	A join semi-lattice, semi-lattice (SL) for short, is a PO such that any non-empty finite  subset of $A$ has a
...
\begin{definition}[monoid]
	A commutative monoid, monoid for short, is a triple
...
%	\end{itemize}
	...
	It is now easy to show that distributive semi-lattice monoids are \emph{tropical} semirings, also called dioids, namely, semirings with a sum operator
...
The fist one is the semi-lattice monoid 
...
The following lemma states that residuation intuitively conveys the meaning of 
...
Moving to ReSL, next lemma ensures that residuation implies distributivity.
...
\begin{definition}[cylindrification]\label{cyli}
	...
	$\exists: 2_f^V \times A \rightarrow A$ such that for all $X\in 2_f^V$
	\begin{enumerate}
...
Since by Definition \ref{pomo} we have $\exists(\emptyset, a) = a$, the requirements of Definition \ref{cyli} trivially hold 
...
\begin{definition}[polyadification]
	...
	such that for all $a, b \in A$, $X \in 2_f^V$, and $\sigma, \tau\in F(V)$
	\begin{enumerate}
...
The lemma helps to ensure that polyadic substitution behaves correctly with respect to $\exists$. 
...
&=& (\exists_{\sigma^c(X)} c) (\eta \circ \sigma)\\
& =& (s_\sigma \exists_{\sigma^c(X)} c) \eta
\end{eqnarray*}
...
%	Let $X\subseteq V$ be a finite set of variables. 
	%
	A constraint $c$ is \emph{affine} if there exist $p \in \mathbb{N}_f^V$ and $i \in A$ 
...
Finally, note that the ReSL $\mathbb{N}_f^V$ of functions $p: V \rightarrow \mathbb{N}$
...
properties of $2_f^V$ and $F(V)$ monoids and to generalise the notion of substitution,
